...
# 改进点：从配置文件导入所有配置，使主代码干净且安全
try:
    import config
except ImportError:
    print("错误：找不到配置文件 aiconfig.py。请确保该文件存在且配置正确。")
    exit()
...
def get_emails(start_date, end_date):
    ...
    (已最终修复日期获取逻辑)
    """
    print(f"正在连接到IMAP服务器 {config.IMAP_SERVER}...")
    ...
    try:
        ...
        mail.login(config.USER_EMAIL, config.PASSWORD)
        mail.select('INBOX')
        print("IMAP连接成功。")
...
def summarize_with_ai(emails_list):
    """调用AI API总结邮件内容。"""
    if not emails_list:
        return ""
    ...
    for i, mail in enumerate(emails_list):
        # 改进点：截断长度可以更长，因为内容更干净了
        content_snippet = mail.get('content', '')[:500]
        if len(mail.get('content', '')) > 500:
            content_snippet += '...'
    ...
    system_prompt = (
        ...
        "报告格式如下：\n\n"
        f"今日共收到 {len(emails_list)} 封重要邮件，其中x封需回复，摘要如下：\n\n"
...
def push_to_weixin(content):
    """推送消息到企业微信。"""
    if not content:
        print("内容为空，不执行推送。")
        return

    print("正在获取企业微信 access_token...")
    try:
        # 改进点：为所有网络请求添加超时和错误处理
        token_url = f"https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid={config.WEIXIN_CORP_ID}&corpsecret={config.WEIXIN_CORP_SECRET}"
        res = requests.get(token_url, timeout=10).json()
        if "access_token" not in res:
             print(f"[错误] 获取access_token失败: {res}")
             return

        access_token = res['access_token']
        push_url = f"https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token={access_token}"
        data = {
            "touser": config.WEIXIN_TO_USER,
            "msgtype": "text",
            "agentid": config.WEIXIN_AGENT_ID,
        ...
        if response.json().get("errcode") == 0:
            ...
            print(f"[错误] 企业微信消息推送失败: {response.text}")

    except requests.exceptions.RequestException as e:
        print(f"[错误] 推送企业微信时发生网络错误: {e}")
    except Exception as e:
        print(f"[错误] 推送企业微信时发生未知错误: {e}")
...
def main():
    ...
    emails = get_emails(start_date=today, end_date=today)
    
    # 2. 生成总结
    if not emails:
        summary = f"{today.strftime('%Y-%m-%d')} 未收到新邮件。"
    else:
        summary = summarize_with_ai(emails)
