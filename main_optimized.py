...
"""
邮件管家 - 优化版本
Email Manager - Optimized Version
"""
...
try:
    ...
    logger.error("配置文件 config.py 未找到")
    exit(1)
...
class EmailConfig:
    ...
    def validate(self) -> bool:
        """验证必要配置"""
        required_vars = [
            'USER_EMAIL', 'PASSWORD', 'IMAP_SERVER',
            'AI_API_KEY', 'WEIXIN_CORP_ID', 
            'WEIXIN_CORP_SECRET', 'WEIXIN_AGENT_ID'
        ]
        
        for var in required_vars:
            if not hasattr(config, var):
                logger.error(f"缺少必要配置: {var}")
                return False
        return True
...
class IMAPClient:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.mail.login(config.USER_EMAIL, config.PASSWORD)
    ...
    def search_emails_by_date(self, start_date: datetime.date, end_date: datetime.date) -> List[str]:
        ...
        try:
            ...
            # 限制扫描范围
            target_ids = email_ids[-self.config.max_emails_to_scan:]
    ...
    def send_message(self, content: str) -> bool:
        ...
        try:
            ...
            token_url = (
                ...
                f"corpid={config.WEIXIN_CORP_ID}&corpsecret={config.WEIXIN_CORP_SECRET}"
            )
